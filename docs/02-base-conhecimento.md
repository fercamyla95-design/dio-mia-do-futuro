...
| Arquivo | Formato | Utilização no Agente MIA |
|---------|---------|---------------------|
...
| `perfil_investidor.json` | JSON | Personalizar recomendações sobre as nescecidades do cliente |
| `produtos_financeiros.json` | JSON | Sugerir produtos adequados ao perfil do cliente |
| `transacoes.csv` | CSV | Analisar padrão de gastos do cliente e usar as informaçoes de forma didadica |
...
#CSV
histórico = pd.read_csv('data/histórico_atendimento.csv)
transacoes = pd.read_csv('data/transacoes.csv')

#JSONs
with open('data/perfil_investidor.json', 'f', encondig-'utf-8') as f
    perfil - json.load(f)
with open('data/produto_financeiro.json', 'f', encondig-'utf-8') as f
    produto - json.load(f)
"""
